...
load('microCT_image_for_stats');

A=img;

% A=A(100:end,100:end);   	% Cut external black frame - depending on image
...
A_rot=imrotate(A,rad2deg(pi/2-alphag),'nearest','crop');
...
figure(3);
A_rescaled=A_rescaled(300:500,:); % Chop external borders
imshow(A_rescaled,[]);
...
A_circle=imrotate(A_rescaled,25,'nearest','crop');
% A_circle=A_circle(100:360,:);
...
com_uct=com_uct*700/mean([ax1_ext ax2_ext]);
...
plot(com_uct(1,1),com_uct(1,2),'r*')
plot(com_uct(2,1),com_uct(2,2),'bs')
plot(com_uct(3,1),com_uct(3,2),'g^')
legend('Ext','T','S','P')
